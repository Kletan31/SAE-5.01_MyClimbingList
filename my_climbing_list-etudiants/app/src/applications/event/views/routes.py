...
from applications.event.models import EventRoute
from applications.event.permissions import (
...
def redirect_to_route_list(event_slug, route_id=None):
    url = reverse("event:staff_route_list", kwargs={"slug": event_slug})
...
def staff_route_list(request, slug):
    ...
    if request.method == "POST":
        ...
        if selected_ids:
            ...
            return redirect("event:staff_route_list", slug=event.slug)

    return render(
        request,
        "event/staff/route_list.html",
        {
            "event": event,
            "routes": routes,
...
def staff_delete_route(request, route_id):
    event_route = get_object_or_404(
        EventRoute.objects.select_related("event"),
        pk=route_id,
        event__salle=get_staff_salle(request),
    )

    if request.method == "POST":
        deleted_route_id = event_route.id
        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": True,
                    "route_id": deleted_route_id,
                }
            )

        return redirect("event:staff_route_list", slug=event_slug)

    return redirect("event:staff_route_list", slug=event_route.event.slug)
...
@event_staff_required
def staff_toggle_route_zone(request, route_id):
    event_route = get_object_or_404(
        EventRoute.objects.select_related("event"),
        pk=route_id,
        event__salle=get_staff_salle(request),
    ...
    if request.method == "POST":
        event_route.has_zone = not event_route.has_zone
        event_route.save(update_fields=["has_zone"])

        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": True,
                    "route_id": event_route.id,
                    "has_zone": event_route.has_zone,
                    "label": "Oui" if event_route.has_zone else "Non",
                }
            )
